...
const postAnime = (anime) => {
  return new Promise((resolve, reject) => {
    connection.query(`insert into savedlist(title, title_Japanese, url, type, episodes, mal_id, rating) values ('${anime.title}', '${anime.title_Japanese}', '${anime.url}', '${anime.type}', ${anime.episodes}, ${anime.mal_id}, '${anime.rating}');`,(err, data) => {
      if(err){
